...
from .models import PhotoModel, Stream
...
# NEW import the necessary packages
...
shape_predictor = dlib.shape_predictor('catalog/shape_predictor_68_face_landmarks.dat/shape_predictor_68_face_landmarks.dat')
# Get the face recognition model
# This is what gives us the face encodings (numbers that identify the face of a particular person)
face_recognition_model = dlib.face_recognition_model_v1('catalog/dlib_face_recognition_resnet_model_v1.dat/dlib_face_recognition_resnet_model_v1.dat')
...
#for test image
def get_test_face_encodings(image):
    # Load image using scipy
    image = imageio.imread(path_to_image)
    # Detect faces using the face detector
    detected_faces = face_detector(image, 1)
...
paths_to_folders = 'catalog/images/'
...
def recognize(request):
    ...
    if request.method == "GET":
        ...
        # data for trained images
        labels, face_encodings = get_images_and_labels(paths_to_folders)

        # data for test image
        face_encodings_in_image = get_test_face_encodings(image)
        # Make sure there's exactly one face in the image
        if len(face_encodings_in_image) != 1:
        ...
        result = 'Idenfitied:', path_to_image, match
"""
# Get path to all the test images
# Filtering on .jpg extension - so this will only work with JPEG images ending with .jpg
test_filenames = filter(lambda x: x.endswith('.jpg'), os.listdir('catalog/test/'))
# Get full paths to test images
paths_to_test_images = ['catalog/test/' + x for x in test_filenames]
...
for path_to_image in paths_to_test_images:
    ...
    face_encodings_in_image = get_test_face_encodings(path_to_image)
    # Make sure there's exactly one face in the image
    if len(face_encodings_in_image) != 1:
        print("Please change image: " + path_to_image + " - it has " + str(
            len(face_encodings_in_image)) + " faces; it can only have one")
